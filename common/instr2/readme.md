...
Отладка — это процесс обнаружения, анализа и исправления ошибок в программном коде. Отладчики позволяют отслеживать выполнение программы, останавливать её выполнение при определённых условиях и проверять значения переменных в различных точках кода. 
...
Так программа будет работать, но это плохое решение. Получается, что внешний код должен заботиться о том, как работает функция: какие параметры можно передавать, а какие нельзя. Логика получается размазанной, часть логики во внешнем коде, часть внутри функции. Это особенно не удобно, когда функций много и в них разные условия работы.
