...
В данном случае лучше использовать указатель, а не ссылку по нескольким причинам:
- У вас может быть какой-то объект очень тяжёлый. Например, если result это большой класс, который долго конструируется. Если у вас будет ссылка, то придётся его сконструировать и передать в функцию ссылку, просто чтобы переписать этот класс. 
