...
Так программа будет работать. Но это плохое решение. Получается, что проверку знаменателя мы выносим из функции. В таком случае, внешний код должен заботиться о том, как работает функция: какие параметры можно передавать, а какие нельзя. Логика получается размазанной: часть логики во внешнем коде, часть внутри функции. 
