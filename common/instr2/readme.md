...
2.[Коды возврата](https://github.com/netology-code/cppm-homeworks/blob/main/common/instr2/readme.md#%D0%BE%D1%88%D0%B8%D0%B1%D0%BA%D0%B8-%D0%B8-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA-%D0%BE%D1%88%D0%B8%D0%B1%D0%BE%D0%BA)
   
3.[Исключения](https://github.com/netology-code/cppm-homeworks/blob/main/common/instr2/readme.md#%D0%BE%D1%88%D0%B8%D0%B1%D0%BA%D0%B8-%D0%B8-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA-%D0%BE%D1%88%D0%B8%D0%B1%D0%BE%D0%BA)
...
Перечисление — это тип данных, который позволяет создавать набор логически связанных констант. Объявление перечисления происходит с помощью оператора enum.
...
Ещё один способ сообщить пользователю функции о результате её работы — записать его в глобальную переменную. Далле после вызова функции пользователь должен проверить эту переменную. Если в ней ошибки нет, то можно использовать результат работы функции.

Этот способ плох по двум причинам:
- Во-первых, он предполагает использование глобальной переменной, это само по себе плохо. Контролировать доступ к обальной переменной сложно, кто угодно может её поменять. Но этот момент при желании можно обойти.
...
Код возврата тоже не является идеальным решением — пользователь функции может проигнорировать и его, но это уже труднее сделать. Тем не менее, использование кода возврата является наилучшим решением с точки зрения производительности. Коды возврата работают быстрее, чем исключения. 

Однако не является лучшим с точки зрения безопасности и удобства использования. Если вам нужно будет использовать коды возврата внутри программы — используйте enum class.
...
Параметром для `catch` выступает объявление исключения — почти как аргумент в функции, но имя необязательно. Здесь мы должны указать тип исключения, которое мы ждём. Мы можем написать сразу несколько блоков `catch`, чтобы обрабатывать исключения разных типов. Но надо помнить о том, что компилятор пытается привести выброшенное исключение к типу каждого блока `catch` сверху вниз. А это значит, что исключения более общего типа надо писать ниже, чем исключения более конкретного.
...
- В коде очевидно, где возникает ошибка (throw) и где она обрабатывается (catch). Эти блоки чётко разделены.
...
Казалось бы, зачем усложнять, если при обнаружении ошибки можно просто напечатать на экран сообщение?
...
- Остаётся проблема возврата какого-то значения из функции — исключения решают эту проблему, прерывая выполнение. Заведомо ошибочные операции не будут выполняться. Печатание на экран не решает проблему выполнения каких-то особенных действий в случае ошибки.
